...
class convert2Yolo(object):
    ...
    def __init__(self, classes_path, image_dir = "C:/python/JPEG/", anno_dir = "C:/python/label/", label_dir = "C:/python/results/"):
    ...
    def convertCoordinate(self,size, box):
        ...
        h = float(h * dh)
        angle=float(box[4])
        return (round(x,3), round(y,3), round(w,3), round(h,3), round(angle,3))
    ...
    def Udacity2Voc(self, dataSet_dir = "example/udacity/", anno_dir = "label/labels.csv", label_dir = "results/", label_list = None):
        ...
        try:
            for line in l:
                ...
                ymax = convertList[4]
                angle= convertList[5]
                label = convertList[7].split('"')[1]

                if length is 9:
                    state = convertList[8].split('"')[1]
                    label = label + state
                ...
                if not os.path.isfile(result_outpath):
                    ...
                    xml_robndbox.append(xml_xmin)

                    xml_ymin = Element("ymin")
                    xml_ymin.text = str(ymin)
                    xml_robndbox.append(xml_ymin)

                    xml_xmax = Element("xmax")
                    xml_xmax.text = str(xmax)
                    xml_robndbox.append(xml_xmax)

                    xml_ymax = Element("ymax")
                    xml_ymax.text = str(ymax)
                    xml_robndbox.append(xml_ymax)
                    

                    xml_angle = Element("angle")
                    xml_angle.text = str(angle)
                    xml_robndbox.append(xml_angle)
                    xml_object.append(xml_robndbox)
    ...
    def parsingUdacity(self, dataSet_dir = "example/udacity/", anno_dir = "label/labels.csv", label_dir = "results/", label_list = None):
        ...
        try:
            for line in l:
                ...
                b = [float(xmin), float(xmax), float(ymin), float(ymax), float(angle)]
                ...
                # This line is outputing 
                result_outfile.write(" ".join([str(a) for a in bb]) + str(cls_id) + " " + '\n')
                list_file.writelines('%s%s.jpg\n' % (work_dirp, os.path.splitext(image_name)[0]))
                print(
                    color.BOLD + color.RED + "------------------------- CSV Parsing -------------------------" + color.END)
            list_file.close()
            print(
        ...
        try:
            ...
            for xml_name in xml_list:
                ...
                for object in objects:
                    ...
                    xmin = float(robndbox.find('xmin').text)
                    xmax = float(robndbox.find('xmax').text)
                    ymin = float(robndbox.find('ymin').text)
                    ymax = float(robndbox.find('ymax').text)
                    angle = float(robndbox.find('angle').text)

                    b = (float(xmin), float(ymin), float(xmax), float(ymax), float(angle))
                    ...
                    result_outfile.write(str(cls_id) +" "+" ".join([str(a) for a in b]) +  '\n')
